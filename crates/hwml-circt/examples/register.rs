...
use hwml_core::syn::{ConstantId, HSyntax};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let clk = HSyntax::one_rc();
    // Create the data input (using '0')
    let input = HSyntax::zero_rc();

    // Create the $reg primitive
    let reg_name = ConstantId::from_with_db(&db, "reg");
    let reg = HSyntax::hprim_rc(reg_name);

    // Apply $reg to clock and input: ($reg clk) input
    let reg_clk = HSyntax::happlication_rc(reg, clk);
    let reg_clk_input = HSyntax::happlication_rc(reg_clk, input);
