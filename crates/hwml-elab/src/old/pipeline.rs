...
pub fn elaborate_expression<'db>(
    ...
    let typed_term = futures::executor::block_on(async { synth(&mut ctx, expr).await }).unwrap();
    ElaborationResult {
        term: Some(typed_term.subject),
    ...
    program
        ...
        .map(|statement| {
            ...
            match statement {
                ...
                _ => todo!("unhandled statement: {:?}", statement),
            }
        })
        .collect()
}
...
fn elaborate_prim_declaration<'db, 'g>(
    ...
    let Typed {
        subject: ty_term,
        ty: _,
    } = futures::executor::block_on(async { synth(ctx, &prim.ty).await }).unwrap();

    // Evaluate the type to get a Value
    match ctx.eval(&Trusted(ty_term)) {
        Ok(ty_value) => {
...
fn elaborate_def_with_context<'db>(
    ...
    if let Some(ref return_ty) = def.ty {
        ...
        let trusted = futures::executor::block_on(async { synth(&mut ctx, &value_expr).await });
        let Typed {
            subject: term,
            ty: _,
        } = *trusted;
...
fn elaborate_meta_with_context<'db>(
    ...
    let zonked_term = zonk(&*ctx.solver.state.borrow(), &*term.unwrap());
