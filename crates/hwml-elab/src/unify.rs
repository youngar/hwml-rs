...
pub fn fresh_meta<'db>(
    env: SolverEnvironment<'db>,
    ty: RcValue<'db>,
    source_range: Option<SourceRange<'db>>,
) -> RcValue<'db> {
    let id = env.fresh_meta_id(ty.clone(), source_range);
    let substitution = env.local_env().clone();
    Value::metavariable_rc(id, substitution, ty)
}

pub fn constrain_equal_ty<'db>(
    env: SolverEnvironment<'db>,
    lhs: RcValue<'db>,
    rhs: RcValue<'db>,
) -> RcValue<'db> {
...
pub fn antiunify<'db>(
    ...
    rhs: RcValue<'db>,
    ty: RcValue<'db>,
) -> (impl Future<Output = UnifyResult<'db>>, RcValue<'db>) {
...
async fn unify_pi<'db>(
    ...
    let global = &state.borrow().global_env;
    ...
    let var = target_env.push_local_var(source);
    ...
    let target_fut = Box::pin(unify(db, target_env, lhs_target, rhs_target, ty.clone()));
...
async fn unify_lambda<'db>(
    ...
    let var = body_env.push_local_var(pi.source.clone());
    ...
    Box::pin(unify(db, body_env, body1, body2, target)).await
}

/// Eta-expand a lambda on the left side: λx. body == t → body == t x
async fn unify_eta_lambda_left<'db>(
    db: &'db dyn salsa::Database,
    env: SolverEnvironment<'db>,
    ...
    let mut target_env = env;
    let var = target_env.push_local_var(pi.source.clone());
    ...
    Box::pin(unify(db, target_env, lhs_body, rhs_applied, codomain)).await
}

/// Eta-expand a lambda on the right side: t == λx. body → t x == body
async fn unify_eta_lambda_right<'db>(
    db: &'db dyn salsa::Database,
    env: SolverEnvironment<'db>,
    lhs: RcValue<'db>,
    lam: &val::Lambda<'db>,
    ...
    let mut body_env = env;
    let var = body_env.push_local_var(pi.source.clone());
    let lhs_body = run_application(global, &lhs, var.clone())?;
    let rhs_body = run_closure(global, &lam.body, [var.clone()])?;
    let body_ty = run_closure(global, &pi.target, [var])?;
    println!("[Unify] Unifying applied term with lambda body");
    Box::pin(unify(db, body_env, lhs_body, rhs_body, body_ty)).await
}

/// Unify HArrow types (injectivity rule)
async fn unify_harrow<'db>(
    db: &'db dyn salsa::Database,
    env: SolverEnvironment<'db>,
    ...
    let mut target_env = env;
    let var = target_env.push_local_var(source.clone());
    ...
    let target_fut = Box::pin(unify(db, target_env, lhs_target, rhs_target, module_ty));
    let (source_result, target_result) = join!(source_fut, target_fut);
    source_result?;
    target_result
}
...
async fn unify_module<'db>(
    ...
    let mut body_env = env;
    let var = body_env.push_local_var(ha.source.clone());
...
// async fn unify_eta_module_left<'db>(
//     db: &'db dyn salsa::Database,
//     mut ctx: SolverEnvironment<'db>,
//     mod1: &hwml_core::val::Module<'db>,
//     rhs: RcValue<'db>,
//     ty: &RcValue<'db>,
// ) -> UnifyResult<'db> {
...
// async fn unify_eta_module_right<'db>(
//     db: &'db dyn salsa::Database,
//     env: SolverEnvironment<'db>,
//     lhs: RcValue<'db>,
//     rhs: &hwml_core::val::Module<'db>,
//     ty: &RcValue<'db>,
// ) -> UnifyResult<'db> {
...
//     let mut body_env = env;
//     let var = body_env.push_local_var(ha.source.clone());
//     let lhs_body = Value::happlication_rc(lhs.clone(), (*ty).clone(), var.clone());
//     let rhs_body = run_closure(
//         &body_env.state.borrow().global_env,
//         &rhs.body,
//         [var.clone()],
//     )?;
//     let body_ty = run_closure(&body_env.state.borrow().global_env, &ha.target, [var])?;
//     println!("[Unify] Unifying applied term with module body");
//     Box::pin(unify(db, body_env, lhs_body, rhs_body, body_ty)).await
// }

/// Unify two Case eliminators
async fn unify_case<'db>(
    db: &'db dyn salsa::Database,
    env: SolverEnvironment<'db>,
    ...
    // // Check parameters count matches.
    ...
    // assert(case1.branches.len() == case2.branches.len());

    // for (b1, b2) in case1.branches.iter().zip(case2.branches.iter()) {
    //     assert(b1.constructor == b2.constructor);

    //     let constructor_info = env.global_env().data_constructor(b1.constructor).unwrap();
    //     assert(b1.arity = info.arity());
    //     assert(b2.arity = constructor_info.arity());

    //     // Extend the environment with variables introduced by the branch pattern.
    //     let arg_env = env.clone();
    //     let mut args = Vec::with_capacity(b1.arity);
    //     for syn_arg_ty in contructor_info. {
    //         let syn_arg_ty = b1.
    //         let var = arg_env.push_local_var(case1.)
    //             .local_values
...
async fn unify_type_constructor<'db>(
    db: &'db dyn salsa::Database,
    ctx: SolverEnvironment<'db>,
...
async fn unify_data_constructor<'db>(
    db: &'db dyn salsa::Database,
    ctx: SolverEnvironment<'db>,
...
async fn unify_happlication<'db>(
    db: &'db dyn salsa::Database,
    ctx: SolverEnvironment<'db>,
...
async fn unify_flex_same<'db>(
    ...
    ctx.set_meta_solution(f1.head.id, solution);

    Ok(())
}

/// Unify two rigid terms (structural equality on head and spine)
async fn unify_rigid<'db>(
    db: &'db dyn salsa::Database,
    ctx: SolverEnvironment<'db>,
...
pub async fn unify<'db>(
    ...
    let ty = whnf(&env, ty).await?;
    let lhs = force(&env, lhs)?;
    let rhs = force(&env, rhs)?;
    ...
    if let Value::Flex(flex) = &*lhs {
        if env.state().is_meta_poisoned(flex.head.id) {
            println!(
    ...
    match (&*lhs, &*rhs) {
        ...
        (Value::Lift(l1), Value::Lift(l2)) => unify_lift(db, env.clone(), l1, l2, ty).await,
        (Value::SLift(s1), Value::SLift(s2)) => unify_slift(db, env.clone(), s1, s2, ty).await,
        (Value::MLift(m1), Value::MLift(m2)) => unify_mlift(db, env.clone(), m1, m2, ty).await,

        // Pi types (injectivity)
        (Value::Pi(pi1), Value::Pi(pi2)) => unify_pi(db, env.clone(), pi1, pi2, ty).await,

        // Lambda terms (injectivity, both sides are lambdas)
        (Value::Lambda(lam1), Value::Lambda(lam2)) => {
            unify_lambda(db, env.clone(), lam1, lam2, &ty).await
        }
        ...
        (Value::Flex(f1), Value::Flex(f2)) if f1.head.id == f2.head.id => {
            unify_flex_same(db, env.clone(), f1, f2).await
        }
        ...
        (Value::Flex(flex), _) if flex.spine.is_empty() => {
            ...
            let depth = env.depth();
            Box::pin(try_solve(db, env, depth, &flex.head, rhs, &ty)).await
        }
        // Symmetric case
        (_, Value::Flex(flex)) if flex.spine.is_empty() => {
            ...
            let depth = env.depth();
            Box::pin(try_solve(db, env, depth, &flex.head, lhs, &ty)).await
        }

        // Eta-expansion: Lambda on left, non-lambda on right
        (Value::Lambda(lam), _) => unify_eta_lambda_left(db, env.clone(), lam, rhs, &ty).await,

        // Eta-expansion: Lambda on right, non-lambda on left
        (_, Value::Lambda(lam)) => unify_eta_lambda_right(db, env.clone(), lhs, lam, &ty).await,

        // HArrow types (injectivity)
        (Value::HArrow(ha1), Value::HArrow(ha2)) => {
            unify_harrow(db, env.clone(), ha1, ha2, ty).await
        }

        // Module terms (injectivity, both sides are modules)
        (Value::Module(mod1), Value::Module(mod2)) => {
            unify_module(db, env.clone(), mod1, mod2, &ty).await
        }

        // // Eta-expansion: Module on left, non-module on right
        // (Value::Module(mod1), _) => unify_eta_module_left(db, env.clone(), mod1, rhs, &ty).await,

        // // Eta-expansion: Module on right, non-module on left
        // (_, Value::Module(mod2)) => unify_eta_module_right(db, env.clone(), lhs, mod2, &ty).await,

        // Type constructors (injectivity)
        (Value::TypeConstructor(tc1), Value::TypeConstructor(tc2)) => {
            unify_type_constructor(db, env.clone(), tc1, tc2).await
        }

        // Data constructors (injectivity)
        (Value::DataConstructor(dc1), Value::DataConstructor(dc2)) => {
            unify_data_constructor(db, env.clone(), dc1, dc2, &ty).await
        }

        // HApplication (structural equality)
        (Value::HApplication(ha1), Value::HApplication(ha2)) => {
            unify_happlication(db, env.clone(), ha1, ha2, ty).await
        }
        ...
        (Value::Flex(flex), _) => {
            ...
            let lowered = lower_flex(env.clone(), flex.clone()).await?;
            // Now unify the lowered flex with the right side
            Box::pin(unify(db, env, lowered, rhs, ty)).await
        }
        (_, Value::Flex(flex)) => {
            println!(
                "[Unify] Flex {} with non-empty spine, lowering first",
            ...
            Box::pin(unify(db, env, lhs, lowered, ty)).await
        }
        (Value::Rigid(r1), Value::Rigid(r2)) => unify_rigid(db, env, r1, r2).await,
        _ => Err(UnificationError::Mismatch(lhs, rhs)),
    }
}

/// Unify two eliminators (applications, projections, etc.)
async fn unify_eliminator<'db>(
    db: &'db dyn salsa::Database,
    ctx: SolverEnvironment<'db>,
    lhs: &Eliminator<'db>,
    rhs: &Eliminator<'db>,
            ...
            hwml_core::val::Eliminator::Application(app) => {
                ...
                match forced_ty.as_ref() {
                    Value::Pi(pi) => {
                        ...
                        ctx.set_meta_solution(current_meta_id, lambda_term);
...
async fn try_solve<'db>(
    ...
    ctx.set_meta_solution(meta_variable.id, syn_solution);
    Ok(())
}
...
//         db: &'db dyn salsa::Database,
//         global: &mut hwml_core::GlobalEnv<'db>,
//     ) {
...
//         let db = Database::default();
//         let mut global = hwml_core::GlobalEnv::new();
